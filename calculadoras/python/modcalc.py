...
def operate(lst):
    ...
    for (i, elem) in enumerate(lst):
        if elem == '*' or elem == 'x' or elem == '/' or elem == ':':
           if is_num(lst[i-1]) is True and is_num(lst[i+1]) is True:
                if elem == '*' or elem == 'x':
            ...
            if i > 0 and i < len(st):
                if st[i] == ' ':
                    if st[i-1] == ' ':
                        st = st[:i-1] + st[i:]
    if st[0] == ' ':
        st = st[1:] #elimina el espacio que ocupa la primera posicion del string
    if st[len(st) - 1] == ' ':
        st = st[:len(st) - 1] #elimina el espacio que ocupa la ultima posicion
    return st
